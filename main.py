    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for item in job_list:
                ...
                # 원하시던 정확한 '어제(00:00~23:59)' 등록 공고만 추출
                if post_date == yesterday_str:
                    ...
                    close_date = (datetime.utcfromtimestamp(int(close_time)) + timedelta(hours=9)).strftime("%Y-%m-%d") if close_date else "상시채용/채용시 마감"
...
def send_email(jobs_results, receiver_emails, target_tag):
    ...
    msg = MIMEMultipart()
    msg["Subject"] = f"[{target_tag}] {date_str} 신규 채용공고 리포트"
    ...
    try:
        ...
        print(f"✅ [{target_tag}] 리포트 발송 성공! -> {', '.join(receiver_emails)}")
    except Exception as e:
        print(f"❌ [{target_tag}] 리포트 발송 실패: {e}")
...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        TARGET_EXP_CD = "1"       # 신입 단독 (1)
        
        # =====================================================================
        # 💡 [핵심] receivers에 실제 수신할 이메일을 각각 적어주세요!
        # 두 수신처에 같은 메일 주소를 넣으면 해당 메일함으로 2개의 메일이 따로 갑니다.
        # =====================================================================
        email_targets = [
            {
                "tag": "K-POP/기획",
            ...
            {
                "tag": "개발팀",
                "receivers": ["ghnam@saramin.co.kr"],  # 👈 개발 공고 수신 이메일
                "job_groups": [
                    {"name": "IT·개발 전체", "job_cd": "1412"}
                ]
            }
        ]
